...
- 桌面工作台与移动端抽屉统一交互，提供示例图案、视角复位、线框与自动旋转。
- 生成、投影和导出使用 Web Worker；生成与投影支持取消，过期任务不会覆盖新项目。
- 修复分辨率和迭代参数不生效、黑图除零、矩阵转置、网格步长公式及模型尺寸误差。
- 焦散预览基于实际三角网格的 BVH 求交和双界面折射，生成后自动显示；黑底、白光、无色透明透镜，支持光路示意、模型和投影正视三种视图及 PNG 下载。
- 投影显示在朝向光源的受光面，背板不透光；支持理想点光源的距离、横纵偏移、相对光强、接收屏范围和实际距离比例设置。
- STL、OBJ、PLY、STEP、JSON 统一应用缩放和单位；STEP 多面实体经过 OpenCascade 回读验证。
- 报告预览与 HTML 导出使用统一的工作台品牌样式，内嵌 logo、源图像和投影结果，记录光源参数，支持 A4 打印。
...
### 2. 调整透镜参数
- **尺寸设置**: 调整透镜的宽度和高度
- **光学参数**: 设置焦距、折射率、目标距离；算法焦距默认为 1.5 m
- **材料选择**: 选择合适的透镜材料
- **优化设置**: 配置算法参数以获得最佳结果
...
- 投影为基于网格的双界面几何光学近似，支持平行光和理想各向同性点光源；未模拟衍射、色散、菲涅耳损耗、吸收及多次内部反射。图像亮度为相对照度的显示映射，不是绝对辐照度。
- 透镜逆向设计仍按平行光计算；切换点光源只对当前模型重新仿真，不会重新优化成相同的目标图案。接收屏自动范围为估计值，超出范围的光线会被裁掉。
- 逆向设计仍需要实物验证，算法焦距、投影距离和加工精度会影响实际图案；当前生成尺寸固定为 100 x 100 mm。
- STEP 导出的是三角面构成的多面实体，不是光滑 NURBS 曲面；高分辨率导出文件较大。
- G-Code 尚未接入切片器，已禁用占位导出。请将 STL 导入切片软件生成设备指令。
- 项目保存在当前页面内存中，刷新页面会清空。JSON 包含几何与参数，暂不支持项目导入。
...
# 浏览器测试（本地默认使用已安装的 Google Chrome）
npm run test:e2e
...
测试覆盖生成取消、无效图像、参数失效、网格尺寸、折射、各格式回读、报告独立打开，以及桌面和 320/390/768px 布局。算法和导出说明见 [维护记录](docs/MAINTENANCE.md)。
